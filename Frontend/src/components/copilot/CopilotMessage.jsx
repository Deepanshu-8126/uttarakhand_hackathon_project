...
export default function CopilotMessage({ msg, onConfirm, onCancel, onSelectAction, isLatest }) {
  ...
  return (
    ...
    >
      ...
      <div className="copilot-msg__bubble relative group">
        {!isUser && isSpeechSynthesisSupported() && msg.text && (
          <button
            type="button"
            onClick={toggleSpeak}
            title={isSpeaking ? "Stop Speaking" : "Listen (AI Voice)"}
            className={`absolute top-2 right-2 p-1 rounded-md transition-all ${
              isSpeaking 
                ? "bg-forest-green text-white animate-pulse" 
                : "text-gray-400 hover:text-forest-green hover:bg-gray-100 opacity-60 group-hover:opacity-100"
            }`}
          >
