...
export default function AICopilotDrawer({ isOpen, onClose, tripId, pageContext }) {
  ...
  const handleToggleVoice = useCallback(async () => {
    ...
    try {
      ...
      console.error("[VoiceInput] Failed to initialize speech recognition:", err);
      setIsListening(false);
    }
  }, [isListening]);
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    const historyPayload = messages
      .filter(m => (m.role === 'user' || m.role === 'agent' || m.role === 'assistant') && m.id !== 'welcome_init')
      .map(m => ({
      ...
      await streamAgentMessage({
        ...
        onUpdate: (event) => {
          if (event.type === 'status' || event.type === 'tool_status') {
            ...
            executeAgentAction(event.action, { requestId: event.requestId || reqId, navigate });
            ...
            if (event.response?.uiActions && Array.isArray(event.response.uiActions)) {
              for (const act of event.response.uiActions) {
                executeAgentAction(act, { requestId: event.requestId || reqId, navigate });
              }
            }
            ...
            setMessages(prev => {
              ...
              let finalText = streamedAnswer || resp.message || "I am ready to help with your trip planning.";

              // Extract and execute inline agent action if present
              const actionMatch = finalText.match(/\[ACTION:\s*(\{[\s\S]*?\})\s*\]/);
              if (actionMatch) {
                try {
                  const actionObj = JSON.parse(actionMatch[1]);
                  executeAgentAction(actionObj, { requestId: event.requestId || reqId, navigate });
                } catch (e) {
                  console.warn("Failed to parse inline agent action:", e);
                }
              }
              finalText = finalText.replace(/\[ACTION:\s*\{[\s\S]*?\}\s*\]/g, '').trim();
        ...
        if (directRes.success && directRes.response) {
          ...
          if (resp.uiActions && Array.isArray(resp.uiActions)) {
            for (const act of resp.uiActions) {
              executeAgentAction(act, { requestId: reqId, navigate });
            }
          }
  ...
  const handleExpandWorkspace = () => {
    navigate(`/copilot?sessionId=${activeSessionId}${tripId ? `&tripId=${tripId}` : ''}`);
    onClose();
  };
      ...
      <div className={`copilot-drawer ${isOpen ? "copilot-drawer--open" : ""}`} role="complementary" aria-label="AI Travel Copilot">
        ...
        <div className="copilot-header">
          <div className="copilot-header__left">
            ...
            <div>
              <p className="copilot-header__title">Trip Assistant</p>
              <p className="copilot-header__sub">Uttarakhand Mountain Guide &amp; Planner</p>
            </div>
          </div>
        ...
        <div className="copilot-input-area">
          <div className="copilot-input-wrap">
            <textarea
              ...
              placeholder={isListening ? "Listening... Speak your trip query..." : "Ask about your Uttarakhand trip..."}
              value={input}
