...
export const useMapStore = create((set, get) => ({
  ...
  // Smart "Add to Trip" Modal State
  addToTripModalItem: null,
  openAddToTripModal: (item) => set({ addToTripModalItem: item }),
  closeAddToTripModal: () => set({ addToTripModalItem: null }),

  // Global Toast System
  globalToast: null,
  showToast: (message, actionText = null, actionLink = null) => {
    const toast = { id: Date.now(), message, actionText, actionLink };
    set({ globalToast: toast });
    setTimeout(() => {
      if (get().globalToast?.id === toast.id) {
  ...
  // Add Item to a specific day in active trip
  addItemToTripDay: (dayIndex, item) => {
    let session = get().activeTripSession;
    const destName = item.location || item.city || item.district || 'Uttarakhand';

    if (!session || !session.dayPlans || session.dayPlans.length === 0) {
      session = {
        tripId: 'TRIP-' + Date.now().toString(36).toUpperCase(),
        destination: { name: destName, district: item.district || 'Uttarakhand' },
        startingLocation: { name: 'Dehradun, Uttarakhand' },
        duration: '3 Days',
        pace: 'Balanced',
        transport: 'By Car',
        ...
        dayPlans: [
          {
            dayNumber: 1,
            title: `Day 1: Arrival & Transfer to ${destName}`,
            description: `Check-in, settle down, and explore nearby scenic trails.`,
            activities: [],
            stays: [],
            rentals: []
          },
          {
            dayNumber: 2,
            title: `Day 2: ${destName} Sightseeing & Experiences`,
            description: `Full day exploration of key attractions and local viewpoints.`,
            activities: [],
            stays: [],
            rentals: []
          },
          {
            dayNumber: 3,
            title: `Day 3: Mountain Vistas & Return Journey`,
            description: `Morning sunrise, local souvenirs, and return transfer.`,
            activities: [],
            stays: [],
            rentals: []
          }
    ...
    const updatedDays = [...(session.dayPlans || [])];
    const targetIdx = Math.max(0, Math.min(dayIndex, updatedDays.length - 1));
    
    if (!updatedDays[targetIdx]) {
      updatedDays[targetIdx] = {
        dayNumber: targetIdx + 1,
        title: `Day ${targetIdx + 1}: Mountain Exploration`,
        description: 'Scheduled mountain activities and experiences.',
        activities: [],
        stays: [],
        rentals: []
      };
    ...
    updatedDays[targetIdx] = targetDay;
    const updatedSession = { ...session, dayPlans: updatedDays };

    set({ activeTripSession: updatedSession, addToTripModalItem: null });
    try {
      localStorage.setItem('discovery_active_trip', JSON.stringify(updatedSession));
    } catch (e) {
      console.warn('Failed to save updated trip session:', e);
    }

    const dayLabel = targetDay.title ? targetDay.title.split(':')[0] : `Day ${targetIdx + 1}`;
    get().showToast(
      `Added "${item.name}" to ${dayLabel} Itinerary`,
      'View Itinerary →',
      updatedSession.tripId || updatedSession._id ? `/my-trip/${updatedSession.tripId || updatedSession._id}` : '/my-trip'
    );
  }
}));
