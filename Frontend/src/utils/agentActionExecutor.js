...
export function executeAgentAction(action, context = {}) {
  ...
  switch (type) {
    ...
    case 'NAVIGATE': {
      let targetPath = null;
      const target = action.target || action.route;
      if (action.routeKey && ALLOWED_ROUTE_MAP[action.routeKey]) {
        targetPath = ALLOWED_ROUTE_MAP[action.routeKey];
      } else if (target && (isAllowedPath(target) || isAllowedPath(target.split('?')[0]))) {
        targetPath = target;
      }

      if (!targetPath) {
        console.warn(`[AgentActionExecutor] REDIRECT/NAVIGATE rejected for invalid target:`, action);
        return { executed: false, reason: 'Disallowed route target.' };
      }

      // If action contains filters object, serialize to query params
      if (action.filters && typeof action.filters === 'object') {
        ...
        if (qs) {
          targetPath = targetPath.includes('?') ? `${targetPath}&${qs}` : `${targetPath}?${qs}`;
        }
      }
    ...
    case 'SHOW_CONTACT': {
      const phone = action.phone || action.phoneNumber;
      const partner = action.partnerName || action.name || 'Verified Partner';
      if (phone) {
        if (typeof window !== 'undefined') {
          window.alert(`${partner} Contact: ${phone}`);
        }
        return { executed: true, phone, partner };
      }
      return { executed: false, reason: 'No phone number provided' };
    }
    ...
    case 'OPEN_STAY':
    case 'OPEN_STAYS': {
      const loc = action.filters?.location || action.location || action.destination;
      const params = new URLSearchParams();
      if (loc) params.set('location', loc);
      if (action.filters) {
        Object.entries(action.filters).forEach(([k, v]) => {
          if (k !== 'location' && v !== undefined && v !== null && v !== '') params.set(k, String(v));
        });
      }
      const qs = params.toString();
      const target = qs ? `/stays?${qs}` : '/stays';
      if (navigate) {
        navigate(target);
        return { executed: true, target };
      }
    ...
    case 'OPEN_RENTALS': {
      ...
      const qs = params.toString();
      const target = qs ? `/rentals?${qs}` : '/rentals';
      if (navigate) {
        navigate(target);
        return { executed: true, target };
      }
