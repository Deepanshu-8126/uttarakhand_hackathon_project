...
/**
 * High-Altitude Physiology Safety Thresholds for Uttarakhand:
 * Prevents dangerous ascent (>800m vertical gain in 24h above 2,500m) without acclimatization.
 */
export const KNOWN_DESTINATION_ALTITUDES = {
  'kedarnath': 3583,
  'badrinath': 3300,
  'tungnath': 3680,
  'hemkund': 4329,
...
export function getAltitudeMeters(destName = '', fallback = 1400) {
  const clean = (destName || '').toLowerCase().trim();
  for (const [key, alt] of Object.entries(KNOWN_DESTINATION_ALTITUDES)) {
    if (clean.includes(key)) return alt;
  }
  return fallback;
}
...
export function generatePersonalizedTripPlan({
  ...
  // Altitude safety calculations (Audit E-03 fix)
  const startAlt = getAltitudeMeters(startName, 400);
  const destAlt = Number(destination.altitude) || getAltitudeMeters(destName, 1800);
  const altitudeGain = destAlt - startAlt;
  const requiresAcclimatization = destAlt > 2500 && altitudeGain > 800;

  const altitudeSafetyBadge = requiresAcclimatization ? {
    isHighAltitude: true,
    destinationAltitudeMeters: destAlt,
    altitudeGainMeters: altitudeGain,
    riskLevel: destAlt > 3500 ? 'High (AMS Protocol Active)' : 'Moderate',
    advisory: `High-Altitude Zone (${destAlt}m, +${altitudeGain}m gain). Acclimatization pacing enforced. Maintain 3-4L hydration, avoid rapid ascent, and keep warm.`
  } : null;
  ...
  dayPlans.push({
    ...
    stay: destinationStay,
    rental: null,
    altitudeSafetyBadge: altitudeSafetyBadge,
    whyThisDay: [
