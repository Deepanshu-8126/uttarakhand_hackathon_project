...
import EscrowOtpModal from '../components/booking/EscrowOtpModal';
...
import FloatingSosBottomSheet from '../components/safety/FloatingSosBottomSheet';
...
export default function MyTripPage() {
  ...
  const [mobileTab, setMobileTab] = useState('itinerary'); // 'itinerary' | 'map'
  ...
  // Feature 14: Escrow Protection state
  const [showEscrowModal, setShowEscrowModal] = useState(false);
  const [activeEscrowBooking, setActiveEscrowBooking] = useState(() => {
    try {
      const saved = localStorage.getItem('active_escrow_booking');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.warn(e);
    }
    return {
      _id: 'bk-traveler-escrow',
      id: 'bk-traveler-escrow',
      bookingReference: 'DU-2026-UK07',
      checkInOtp: '7294',
      amount: 3200,
      totalAmount: 3200,
      itemTitle: 'Himalayan Eco Stay & Fleet',
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] flex flex-col font-sans">
      ...
      {/* ── 2. TRIP AT A GLANCE ───────────────────────────────────── */}
      <div className="bg-[#f7f5f0] border-b border-slate-200 py-4 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
      ...
      {/* ── ESCROW + OTP HANDSHAKE STATUS CARD ── */}
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-6">
        <div className="p-5 sm:p-6 rounded-2xl bg-[#0f3d2e] border border-emerald-700/60 text-white shadow-lg flex flex-col lg:flex-row lg:items-center justify-between gap-5 relative overflow-hidden">
          <div className="absolute -right-6 -bottom-6 w-36 h-36 bg-emerald-400/10 rounded-full blur-xl pointer-events-none" />

          <div className="space-y-1.5 max-w-2xl">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 text-[10px] font-bold text-emerald-300 bg-white/10 border border-emerald-400/30 rounded-full uppercase tracking-wider flex items-center gap-1">
                <ShieldCheck size={12} className="text-emerald-300" />
                Escrow Protected Booking
              </span>
              <span className="text-[10px] font-semibold text-emerald-200/80">
                Ref: {activeEscrowBooking.bookingReference || 'DU-2026-UK07'}
              </span>
            </div>

            <h4 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
              <span>Funds Protected in Escrow</span>
              <span className="text-xs font-semibold text-emerald-300 bg-emerald-900/80 px-2 py-0.5 rounded border border-emerald-600/40">
                ₹{(activeEscrowBooking.totalAmount || activeEscrowBooking.amount || 3200).toLocaleString('en-IN')} Locked
              </span>
            </h4>
            
            <p className="text-xs text-emerald-100/80 leading-relaxed">
              Step 2 of 3: Meet your verified host or vehicle partner on-site. Only share your secret 4-digit OTP below after verifying the keys and room.
            </p>
          </div>

          <div className="flex items-center gap-3 shrink-0 bg-white/10 backdrop-blur-xs p-3.5 rounded-xl border border-white/15">
            <div className="text-right">
              ...
              <span className="text-2xl sm:text-3xl font-mono font-black text-white tracking-widest">
                {activeEscrowBooking.checkInOtp || '7294'}
              </span>
            </div>
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg bg-emerald-400 hover:bg-emerald-300 text-slate-950 font-bold text-xs transition-all shadow-sm flex items-center gap-1 cursor-pointer"
            >
              <span>Voucher</span>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-1 space-y-10">
        {saveMessage && (
        ...
        {/* ── Mobile View Segmented Tab (<1024px) ── */}
        <div className="lg:hidden flex items-center p-1 bg-stone-200/80 rounded-2xl mb-4 max-w-sm mx-auto shadow-2xs">
          <button
            type="button"
            onClick={() => setMobileTab('itinerary')}
            className={`flex-1 py-2.5 px-3 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 ${
              mobileTab === 'itinerary' 
                ? 'bg-white text-[#0f3d2e] shadow-sm font-black' 
        ...
        {/* Responsive Layout: Mobile Tabs / Desktop Split Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start pb-20 sm:pb-0">
          
          {/* Left Column: Daily Day Cards */}
          <div className={`lg:col-span-7 space-y-6 ${mobileTab === 'itinerary' ? 'block' : 'hidden lg:block'}`}>
            {dayPlans.map((day, idx) => (
              <DayCard
          ...
          <div className={`lg:col-span-5 lg:sticky lg:top-24 space-y-4 ${mobileTab === 'map' ? 'block' : 'hidden lg:block'}`}>
      ...
      {/* Escrow OTP Voucher Modal */}
      <EscrowOtpModal
        booking={activeEscrowBooking}
        isOpen={showEscrowModal}
        onClose={() => setShowEscrowModal(false)}
        onVerified={(updated) => setActiveEscrowBooking({ ...activeEscrowBooking, ...updated })}
      />

      {/* Floating Safety & SOS Grid Bottom Sheet (Winning Safety Feature) */}
      <FloatingSosBottomSheet />
    </div>
  );
}
