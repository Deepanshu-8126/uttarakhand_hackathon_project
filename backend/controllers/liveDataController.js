 ...
 */
export const getCommunityAdvisories = async (req, res) => {
  try {
    const { circuit } = req.query;
    const { getActiveCommunityAdvisories } = await import('../services/communityGridService.js');
    const advisories = await getActiveCommunityAdvisories(circuit);
    return res.status(200).json({
      success: true,
    ...
    return res.status(500).json({
      success: false,
      message: 'Failed to fetch community advisories',
      error: err.message
    });
  }
};
...
/**
 * Feature 10: Submit Community Ground Condition Report
 * POST /api/live/community-grid/report
 */
export const submitCommunityReport = async (req, res) => {
  try {
    const { location, condition, description, coordinates, reporterName, reporterRole, isOfflineSynced } = req.body;
    if (!location || !condition || !description) {
      return res.status(400).json({
        success: false,
        message: 'Location, condition, and description are required.'
      });
    }

    const { recordCommunityReport } = await import('../services/communityGridService.js');
    const report = await recordCommunityReport({
      location,
      condition,
      description,
      coordinates,
      reporterName: reporterName || req.user?.name || 'Local Valley Partner',
      reporterRole: reporterRole || (req.user?.role === 'partner' ? 'PARTNER' : 'TRAVELER'),
      isOfflineSynced
    });

    return res.status(201).json({
      success: true,
      message: report.consensusVerified 
        ? 'Report submitted and verified by 3-partner community consensus!' 
        : 'Report submitted to community grid.',
      data: report
    });
  } catch (err) {
    return res.status(500).json({
      success: false,
      message: 'Failed to record community report',
      error: err.message
    });
  }
};
