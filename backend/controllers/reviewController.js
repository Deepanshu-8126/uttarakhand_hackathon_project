...
export const getReviews = async (req, res) => {
  try {
    ...
    if (targetType) {
      query.targetType = { $regex: new RegExp(`^${targetType}$`, 'i') };
    }
    
    if (targetId && targetId !== 'general' && targetId !== 'all' && targetId !== 'site') {
    ...
    console.error('[ReviewController] getReviews handled gracefully:', error?.message);
    res.status(200).json({ success: true, count: 0, data: [], message: error?.message });
  }
};
