...
async function runPythonAgent({ message, chatId, tripContext, session, user, requestId, pageContext, onEvent }) {
  ...
  try {
    ...
    const pResp = (typeof data.response === 'object' && data.response !== null)
      ? data.response
      : { message: (typeof data.response === 'string' ? data.response : (data.text || data.message || '')) };

    const responseMessage = pResp.message || (typeof data.response === 'string' ? data.response : '') || data.text || data.message || '';

    if (onEvent && responseMessage) {
      onEvent({ type: 'delta', content: responseMessage });
      onEvent({ type: 'chunk', text: responseMessage });
    }

    const canonTools = pResp.toolsUsed || data.tools_used || [];
    const canonToolCount = pResp.toolCount !== undefined ? pResp.toolCount : canonTools.length;
    const canonCitations = pResp.citations || data.citations || [];
    const canonCitationCount = pResp.citationCount !== undefined ? pResp.citationCount : canonCitations.length;
    const canonProvider = pResp.meta?.provider || pResp.providerUsed || data.provider || 'auto';
    const canonFallback = pResp.meta?.fallbackUsed || pResp.fallbackUsed || false;
    const canonConfidence = pResp.confidence || data.confidence || 'grounded';
    const canonUiActions = pResp.uiActions || pResp.ui_actions || data.ui_actions || data.uiActions || [];
    const canonSuggestedActions = pResp.suggestedActions || pResp.suggested_actions || data.suggested_actions || data.suggestedActions || [];
    ...
    return {
      ...
      suggestedActions: canonSuggestedActions,
      uiActions: canonUiActions,
